...
- `TimeSeries.provenance` is copied from the source `Observation.provenance` dcid string.
...
Both entrypoints use the same single source-read path over `Observation`.
...
- The current checked-in local source DDL does not show the `Observation.provenance` column, but this module expects that deployed source schema field to exist.
