...
public class TimeseriesBackfillPipeline {
  ...
  static void validateOptions(TimeseriesBackfillOptions options) {
    ...
    if (options.getMaxSeriesRows() > 0 || options.getMaxPointRows() > 0) {
      throw new IllegalArgumentException(
          "Beam row caps are not supported with partitioned Spanner reads. "
              + "Use observation_about sharding for Beam/Dataflow runs or use the validator for "
              + "small bounded runs.");
    }
  }
  ...
  private static LocalProgressTracker createLocalProgressTracker(
      TimeseriesBackfillOptions options) {
  ...
  private static String runnerName(TimeseriesBackfillOptions options) {
  ...
  static final class SourceRowsToMutationGroupsFn extends DoFn<Struct, MutationGroup> {
    ...
    @ProcessElement
    public void processElement(@Element Struct row, OutputReceiver<MutationGroup> out) {
      SourceObservationRow sourceRow = SourceObservationRows.toObservationRow(row);
